   ...
   /* $(".music-list").on("mousemove",".list-item", function() {
        ...
        if(!$(this).data("loadmenu")) {
            ...
            '<div class="list-menu" data-no="' + num + '">' +
                ...
                '<span class="list-icon icon-add" data-function="add" title="点击添加到我的列表"></span>' +
                '<span class="list-icon icon-like" data-function="like" title="点赞"></span>' +
            '</div>';
            target.html(html);
    ...
    // 列表中的菜单点击
    $(".music-list").on("click",".icon-play,.icon-download,.icon-share, .icon-add, .icon-like", function() {
        var num = parseInt($(this).parent().data("no"));
        if(isNaN(num)) return false;
        switch($(this).data("function")) {
            ...
            case "add":   //  添加
                ...
                var add_music_callback = function(resp) {
                    if (resp.execute_err == "") {
                ...
                addMusic(userId, musicInfo.id, musicInfo.name, "tencent", add_music_callback)
                musicList[1].item.push(musicList[rem.dislist].item[num])
                break;
            case "like":   //  点赞
                //var nas_num = likeit()
                ...
                var add_like_callback = function(resp) {
                    if (resp.execute_err == "") {
                        ret = JSON.parse(resp.result)
                    } else {
                        conosole.log(resp.execute_err)
                ...
                addLike(userId, musicInfo.id, musicInfo.name, "tencent", add_like_callback)
                //TODO 增加点赞列表
            break;
        }
        return true;
    ...
    $('img').on("error", function(){
        $(this).attr('src', '/yueting/images/player_cover.png');
    });
...
function changeCover(music) {
    ...
    if(img == "err") {
        img = "/yueting/images/player_cover.png";
    } else {
...
function loadList(list) {
    ...
    if (false) {
        ...
        var promise;
        if (parseInt(list) == 1) {
            promise = getUserMusicList()
        } else if (parseInt(list) == 3) {
            promise = getRandomMusics()
        } else {
            throw new Error("not support list")
        }
...
function addItem(no, name, auth, album){
    // 直接展示相关操作按钮
    var num = no - 1;
    var nameHtml = '<span class="music-name-cult">' +
        name +
    '</span>' +
    '<div class="list-menu1" data-no="' + num + '">' +
    '<span class="list-icon icon-play" data-function="play" title="点击播放这首歌"></span>' +
    '<span class="list-icon icon-add" data-function="add" title="点击添加到我的列表"></span>' +
    '<span class="list-icon icon-like" data-function="like" title="点赞"></span>' +
    '</div>';
    ...
    '    <span class="music-name">' + nameHtml + '</span>' +
    '</div>';
...
function addListbar(types) {
    ...
    switch(types) {
        ...
        case "nodata":  // 列表中没有内容
            html = '<div class="list-item text-center" id="list-foot">没有喜欢的歌曲，赶快去音乐广场看看</div>';
        break;
...
function refreshList() {
    ...
    if(rem.paused !== true) {   // 没有暂停
        // 如果rem.playid == -1 则获取正在播放列表中的id
        if(rem.playid != undefined && rem.playid != -1){
            ...
            for(var i=0; i<musicList[rem.dislist].item.length; i++) {
                // 与正在播放的歌曲 id 相同
                if((musicList[rem.dislist].item[i].id !== undefined) &&
                    (musicList[rem.dislist].item[i].id == musicList[0].item[rem.curIndex].id) &&
                    (musicList[rem.dislist].item[i].source == musicList[0].item[rem.curIndex].source)) {
                    $(".list-item[data-no='" + i + "']").addClass("list-playing");  // 添加正在播放样式
...
function addSheet(no, name, cover) {
    if(!cover) cover = "yueting/images/player_cover.png";
    if(!name) name = "读取中...";
        ...
        if (i == 1) {    // 正在播放列表
         ...
         // 列表不是用户列表，并且信息为空，需要ajax读取列表
            ...
            if(musicList[i].id) {   // 列表ID已定义
                // ajax获取列表信息 TODO 等待API接口开发完成后再调用
...
function clearDislist() {
    ...
    if(rem.dislist == 1) {  // 正在播放列表
        ...
        $(".sheet-item[data-no='1'] .sheet-cover").attr('src', '/yueting/images/player_cover.png');    // 恢复正在播放的封面

...
//                    "<p data-i18n=swf/name></p>" +
                    ...
                    '<button class="btn btn-block" data-i18n=swf/sign_up></button>'
                    )
                .on("click", "button", onClickUnlock)
                .on("keyup", "input[type=password]", onKeyUpPassword)
                .on("keyup", "input[type=nas_num]", onKeyUpPassword)
                .on({
                    change: onChangeFile,
                    click: onClickFile
                }, "input[type=file]"));
            ...
            function onClickUnlock() {
                ...
                if (mFileJson)
                    if (typeof callback == "function") {
                        //console.log($swf[0], mFileJson, mAccount, $swf.find("input[type=password]").val())
